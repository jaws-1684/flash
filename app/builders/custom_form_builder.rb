class CustomFormBuilder < ActionView::Helpers::FormBuilder
  ...
  TEXT_FIELD_CLASSES = ClassVariants.build(
    base: 'flex w-full border rounded-md py-2 px-3 focus:outline-hidden dark:bg-gray-700/50 dark:border-gray-500 dark:text-gray-300 dark:placeholder-gray-400 dark:focus:ring-2 dark:focus:border-transparent',
    variants: {
      state: {
  ...
  def email_field(attribute, options = {})
    options[:placeholder] ||= "user@email.com"
    default_opts = { class: classes_for(attribute, options) }
  ...
  def password_field(attribute, options = {})
    ...
    default_opts = { class: "#{classes_for(attribute, options)}", "data-toggle-target" => "input" }
    
   @template.content_tag :div, class: "relative", "data-controller" => "toggle" do
        text_layout(attribute) { super(attribute, options.merge(default_opts)) } +
           @template.inline_svg_tag("eye.svg", class: "#{PASSWORD_VISIBILITY_CLASSES.render(class: :base)}", id: "eye", data: {action: "click->toggle#toggle", "toggle-target" => "icon"}) + 
              attribute_error_message(attribute)
    end
  end
  ...
  def submit(value = nil, html_options: {})
    default_opts = { class: "cursor-pointer p-2 #{html_options&.dig(:class)}" }
  ...
  def text_layout(attribute)
    @template.content_tag :div, class: 'mt-2 relative rounded-md shadow-xs' do
      yield 
      # + attribute_error_icon(attribute)
    end
  end
