        ...
        <div className={theme}>
          <div className='flex min-h-screen max-w-screen bg-white dark:text-gray-200 dark:bg-fgray'>
             <div className='notice z-50 absolute top-20 right-10  w-fit'>
              {notice && toast.notice && <Success onClick={() => setToast(toast => ({...toast, notice: false}))} text={notice}/>}
              {alert && toast.alert && <Danger onClick={() => setToast(toast => ({...toast, alert: false}))} text={alert}/>}
            </div>
