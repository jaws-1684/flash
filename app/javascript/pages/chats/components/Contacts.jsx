...
function Contacts({chats, favorite}) {
   return (
    <>
    { favorite && <Link key={favorite.id} href={jsRoutes.chatMessagesPath(favorite.id)}>
            <Action>  
                <Avatar className="size-12" avatar={favorite.avatar} alt="chat avatar"/>
    ...
    {chats.filter(chat => chat.id !== favorite?.id).map(chat =>
