...
function Result({searchTerm, loading, contacts}) {
    if (!searchTerm) return (<div className='flex flex-col items-center justify-center gap-2 mt-1'>
                  <ContactBook className="size-xs fill-gray-800 mt-14"/>
                  <div>
                    <h1 className='text-5xl font-bold text-gray-800 text-center'>Find People</h1>
                    <p className='inline-flex items-center text-xl text-gray-700 text-center'>Search above to connect with your friends.</p>
                  </div>
                </div>)
...
export default function Contacts() {
  ...
  return (
    <>
    <Search searchTerm={searchTerm} setSearchTerm={setSearchTerm} placeholder="You can find people here"/>
    <Result searchTerm={searchTerm} loading={contactsLoading} contacts={contacts}/>    
    </>
  )
}
